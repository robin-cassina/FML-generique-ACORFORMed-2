'''
    ...
    Permet de faire un état des lieux des TorsoGestures disponibles
    
    Entrée : fichier .xml d'un lexique de gestes issus de
        /BehaviorRealizer/BehaviorLexicon/TorsoGestures.xml
    Sorties : 
...
file_in = open("../../../../greta-master/bin/BehaviorRealizer/BehaviorLexicon/TorsoGestures.xml", "r", encoding="utf-8")
...
regex = re.compile(r"<([a-zA-Z]+) id=\"([\w_]+)\" lexeme=\"([\w_]+)\" .+>")
...
file_html = open("Torso_Gestures-list.html", "w", encoding="utf-8")
...
file_out = open("Torso_Gestures-Planner.xml", "w", encoding="utf-8")
...
for classe in gestes :
    ...
    for categorie in gestes[classe] :
        ...
        #                   (/!\ réécrit à chaque lancement)
        file_FML = open("Torso_Gestures-FMLs/FML_"+categorie+".xml", "w",
                                        encoding="utf-8")
