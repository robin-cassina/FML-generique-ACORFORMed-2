'''
    ...
    Permet de faire un état des lieux des facelibrary disponibles
    
    Entrée : fichier .xml d'un lexique de gestes issus de
        /BehaviorRealizer/BehaviorLexicon/facelibrary.xml
    Sorties : 
...
file_in = open("../../../../greta-master/bin/BehaviorRealizer/BehaviorLexicon/facelibrary.xml", "r", encoding="utf-8")
...
regex = re.compile(r"<([a-zA-Z]+) class=\"([a-zA-Z]+)\" instance=\"([\w_]+)\">")
not_au = re.compile(r"AU\d+")
...
file_html = open("Face_Library-List.html", "w", encoding="utf-8")

# on écrit l'en-tête, le titre, le paragraphe notant l'essentiel
file_html.write("<!DOCTYPE html>\n<html lang=\"fr\">\n"
                "<head>\n<meta charset=\"utf-8\">\n</head>\n<body>\n"
                "\t<h1>Catalogue des expressions disponibles pour Camille</h1>\n"
...
file_out = open("Face_Library-Planner.xml", "w", encoding="utf-8")
...
for classe in gestes :
    ...
    file_out.write('<!------ FACE EXPRESSION ------>\n')
    ...
    for categorie in gestes[classe] :
        ...
        for nom in liste_noms :
            ...
            # le nom du comportement est catalog-CategorieNumero
...
for classe in gestes :
    ...
    for categorie in gestes[classe] :
        ...
        #                   (/!\ réécrit à chaque lancement)
        file_FML = open("Face_Library-FMLs/FML_"+categorie+".xml", "w",
                                        encoding="utf-8")
        ...
        while tm <= nbnom :
            ...
            file_FML.write('"/>\n')
            file_FML.write('\t\t\t\tCette expression est la numéro ')
